...
// Permite usar BASE absoluta si definiste NEXT_PUBLIC_API_URL; si no, va relativo (proxy/rewrites)
const BASE =
...
// 🔧 Ajusta el mapeo a diferentes shapes de backend
function normalizeOrders(raw: any): Order[] {
  ...
  return list
    .map((r) => {
      ...
      return {
        id: String(r.id ?? r.orderId ?? r.uuid ?? cryptoRandomId()),
        number:
          r.number ?? r.orderNumber ?? r.code ?? `#${String(r.id ?? "").padStart(6, "0")}`,
        createdAt: String(r.createdAt ?? r.date ?? r.placedAt ?? new Date().toISOString()),
        status: String(r.status ?? r.state ?? "Delivered"),
        total: Number(r.total ?? r.totalPrice ?? r.amount ?? items.reduce((s, it) => s + it.unitPrice * it.quantity, 0)),
        items,
      } as Order;
    })
    .filter((o) => !!o.id);
...
export default function OrderHistory() {
    ...
    (async () => {
      ...
      try {
        const res = await fetch(apiUrl("/api/v1/orders/all"), {
          cache: "no-store",
          // credentials: "include", // descomenta si tu API usa cookies/sesión
        });
  ...
  const currency = "USD"; // ajusta si tu backend trae moneda
    ...
    if (err) {
      return (
        <div className="text-sm text-red-600">
          Error: {err}. Verifica que <code>/api/v1/orders/all</code> exista y devuelva JSON.
        </div>
      );
    }
            ...
            {orders.map((o) => {
              ...
              return (
                <tr key={o.id} className="bg-white rounded-lg shadow-sm">
                  <td className="px-3 py-3 font-medium">{o.number ?? `#${o.id}`}</td>
                  <td className="px-3 py-3">{formatDate(o.createdAt)}</td>
                  <td className="px-3 py-3">{o.status}</td>
                  <td className="px-3 py-3">{money(o.total, currency)}</td>
                            ...
                            {o.items.map((it) => (
                              <li key={it.id} className="flex items-center gap-3">
                                ...
                                <div className="text-sm font-medium">
                                  {money(it.unitPrice * it.quantity, currency)}
                                </div>
                              </li>
                            ))}
                          </ul>
